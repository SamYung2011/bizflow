...
function renderKnowledge(state, helpers, t) {
  ...
  return section(e(t("knowledgeTitle")), `<div class="wa-editor-meta"><span class="wa-dirty${dirty ? " is-dirty" : ""}" data-wa-knowledge-status>${e(t(dirty ? "unsaved" : "saved"))}</span><span data-wa-knowledge-count>${e(t("chars", { count: String(state.settings.knowledge || "").length }))}</span></div>${textarea({ e, liveReadOnly: state.liveReadOnly, label: t("knowledgeTitle"), value: state.settings.knowledge, field: "knowledge", rows: 24 })}<div class="wa-button-row"><button type="button" class="wa-button wa-button--secondary" data-wa-knowledge-cancel${writeAttributes(state.liveReadOnly, !dirty)}>${e(t("cancelChanges"))}</button><button type="button" class="wa-button" data-wa-save="knowledge"${writeAttributes(state.liveReadOnly, !dirty)}>${e(t("saveLocal"))}</button></div>`, "wa-panel--editor");
}

function renderChargerPrompt(state, helpers, t) {
  const { escapeHtml: e } = helpers;
  const valid = promptPlaceholdersValid(state.settings.locationHintPrompt);
  return section(e(t("chargerPromptTitle")), `${textarea({ e, liveReadOnly: state.liveReadOnly, label: t("chargerPromptTitle"), value: state.settings.chargersPrompt, field: "chargersPrompt", rows: 12 })}${textarea({ e, liveReadOnly: state.liveReadOnly, label: t("locationPromptTitle"), value: state.settings.locationHintPrompt, field: "locationHintPrompt", rows: 16 })}<p class="wa-validation${valid ? " is-valid" : " is-invalid"}" data-wa-prompt-validation>${e(t(valid ? "promptValid" : "placeholderMissing"))}</p><button type="button" class="wa-button" data-wa-save="prompts"${writeAttributes(state.liveReadOnly, !valid)}>${e(t("saveLocal"))}</button>`, "wa-panel--editor");
}
...
function renderWhitelist(state, helpers, t) {
  ...
  return `<div class="wa-whitelist-grid">${whitelistKinds.map((kind) => {
    ...
    return section(e(t(labels[kind])), `<ul class="wa-whitelist-list">${rows.map((row) => `<li><label class="wa-toggle-row"><input type="checkbox" data-wa-whitelist-toggle="${e(row.id)}"${row.active ? " checked" : ""}${writeAttributes(state.liveReadOnly)}><span>${e(t("active"))}</span></label><div><strong>${e(row.value || t("empty"))}</strong><small>${e(row.note || t("empty"))}</small></div><button type="button" class="wa-icon-button" data-wa-whitelist-remove="${e(row.id)}" data-wa-write title="${e(t("remove"))}"${state.liveReadOnly ? ' disabled aria-disabled="true"' : ""}>×</button></li>`).join("")}</ul><div class="wa-whitelist-add"><input value="${e(draft.value)}" data-wa-whitelist-draft="value" data-wa-whitelist-kind="${kind}" data-wa-write placeholder="${e(t("value"))}"${state.liveReadOnly ? ' disabled aria-disabled="true"' : ""}><input value="${e(draft.note)}" data-wa-whitelist-draft="note" data-wa-whitelist-kind="${kind}" data-wa-write placeholder="${e(t("note"))}"${state.liveReadOnly ? ' disabled aria-disabled="true"' : ""}><button type="button" class="wa-button" data-wa-whitelist-add="${kind}"${writeAttributes(state.liveReadOnly, !draft.value.trim())}>${e(t("add"))}</button></div>`);
  }).join("")}</div>`;
}
