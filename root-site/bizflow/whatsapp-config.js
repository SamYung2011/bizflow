...
function field({ e, label, value = "", field, dataAttribute = "", type = "text", hint = "", disabled = false, liveReadOnly = false, min = null, max = null, autocomplete = "" }) {
  ...
  return `<label class="wa-field"><span>${e(label)}</span><input type="${type}" value="${e(value)}"${binding}${attributes}${autocomplete ? ` autocomplete="${e(autocomplete)}"` : ""}${min === null ? "" : ` min="${e(min)}"`}${max === null ? "" : ` max="${e(max)}"`}>${hint ? `<small>${e(hint)}</small>` : ""}</label>`;
}
