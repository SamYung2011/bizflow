...
function renderConversations(state, helpers, t, dateFilter) {
  ...
  return `${filterBar({ stateKey: "conversation", value: state.conversationChannel, helpers, t, dateFilter })}<div class="wa-chat-layout" data-wa-conversation-count="${conversations.length}"><aside class="wa-conversation-list" data-scroll-restore="bizflow.whatsapp.conversations">${list}</aside><section class="wa-chat-flow" data-scroll-restore="bizflow.whatsapp.messages" data-wa-selected-bubbles="${selected?.rows.length || 0}">${flow}</section></div>`;
}
...
function renderReplies(state, helpers, t) {
  ...
  const rows = (items, isPending) => items.map((row) => `<li class="wa-queue-row"><div><strong>${e(row.chatName || row.customerId || t("empty"))}</strong><p>${e(replyPreview(row.segments) || t("empty"))}</p><span class="wa-channel wa-channel--${channelOf(row)}">${e(t(channelKey(channelOf(row))))}</span></div><div><time>${e(row.time || t("empty"))}</time>${isPending ? `<button type="button" class="wa-button wa-button--secondary" data-wa-reply-skip="${e(row.id)}" data-wa-write${state.activityReadOnly || state.writeBlocked ? ' disabled aria-disabled="true"' : ""}>${e(t("skip"))}</button>` : `<small>${e(t("deliveredAt"))}: ${e(row.deliveredAt)}</small>`}</div></li>`).join("");
