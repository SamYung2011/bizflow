...
function renderReplies(state, helpers, t) {
  ...
  const rows = (items, isPending) => items.map((row) => `<li class="wa-queue-row"><div><strong>${e(row.chatName || row.customerId || t("empty"))}</strong><p>${e(replyPreview(row.segments) || t("empty"))}</p><span class="wa-channel wa-channel--${channelOf(row)}">${e(t(channelKey(channelOf(row))))}</span></div><div><time>${e(row.time || t("empty"))}</time>${isPending ? `<button type="button" class="wa-button wa-button--secondary" data-wa-reply-skip="${e(row.id)}" data-wa-write${state.activityReadOnly ? ' disabled aria-disabled="true"' : ""}>${e(t("skip"))}</button>` : `<small>${e(t("deliveredAt"))}: ${e(row.deliveredAt)}</small>`}</div></li>`).join("");
