...
export function renderTaskToolbar({ state, filterState, members, featureAiBatch, helpers }) {
  ...
  return `<div class="team-kanban-toolbar team-domain-toolbar">
    <div class="team-kanban-filters"><label class="team-task-search">${icon("icon-nav-search", "icon")}<input type="search" data-task-search value="${escapeHtml(filterState.search)}" placeholder="${escapeHtml(taskT(lang, "tasks.search"))}" aria-label="${escapeHtml(taskT(lang, "tasks.search"))}" autocomplete="off"></label>${memberFilter}${boardFilters}${renderTaskFilter({ group: "view", filterState, members, helpers })}<label class="team-only-mine"><input type="checkbox" data-task-only-mine${state.onlyMine ? " checked" : ""}><span>${escapeHtml(taskT(lang, "tasks.onlyMine"))}</span></label></div>
    <div class="team-domain-toolbar__actions">${featureAiBatch && state.permissions.canCreate ? `<button type="button" class="team-ai-task" data-task-ai-open${state.writeBusy || (state.liveReadOnly && !state.liveTaskWrites) ? " disabled aria-disabled=\"true\"" : ""}>${icon("icon-task-list", "icon")}<span>${escapeHtml(taskT(lang, "tasks.ai.open"))}</span></button>` : ""}${state.permissions.canCreate ? `<button type="button" class="team-new-task" data-task-submit-open${state.writeBusy || (state.liveReadOnly && !state.liveTaskWrites) ? " disabled aria-disabled=\"true\"" : ""}>${icon("icon-add-line-add")}<span>${escapeHtml(taskT(lang, "tasks.new"))}</span></button>` : ""}</div>
  </div>`;
}
