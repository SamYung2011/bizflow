...
export function renderTaskActionPopover({ task, open, state, helpers }) {
  ...
  return `<div class="task-action-popover" role="menu" tabindex="-1" data-task-action-popover data-task-id="${escapeHtml(task.id)}" aria-label="${escapeHtml(taskT(lang, "tasks.action.menu"))}">
    ...
    ${canComplete ? `<button type="button" role="menuitem" data-task-action-complete="${escapeHtml(task.id)}"${!completion.canToggle || completion.checked || isWaitingApproval(task) ? " disabled aria-disabled=\"true\"" : completeDisabled}>${escapeHtml(taskT(lang, "tasks.action.complete"))}</button>` : ""}
