...
function renderComment(comment, entryId, state, helpers) {
  ...
  return `<article class="member-update-comment" data-update-comment="${escapeHtml(comment.id)}">
    ...
    <button type="button" data-update-comment-delete="${escapeHtml(comment.id)}" data-update-id="${escapeHtml(entryId)}" aria-label="${escapeHtml(memberT(lang, "members.updates.delete"))}"${state.liveReadOnly ? " disabled" : ""}>×</button>
  </article>`;
}
...
function renderUpdate(entry, state, helpers) {
  ...
  const tt = (key) => memberT(lang, key);
  if (state.editingUpdateId === entry.id) return renderUpdateForm({ entry, state, helpers });
  return `<article class="member-update-card" data-update-log="${escapeHtml(entry.id)}">
    <header>
      ...
      <div class="member-update-card__actions">
...
export function renderMemberUpdateLogs({ state, helpers }) {
  return `<section class="member-updates" data-member-updates data-update-count="${state.updateLogs.length}">
    ${renderUpdateForm({ state, helpers })}
    <div class="member-update-timeline">${state.updateLogs.map((entry) => renderUpdate(entry, state, helpers)).join("")}</div>
  </section>`;
}

export function attachMemberUpdateLogController({ state, currentUserName, rerender }) {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (remove && window.confirm(memberT(document.documentElement.lang === "zh-Hant" ? "zh" : document.documentElement.lang, "members.updates.confirmDelete"))) {
    ...
    if (removeComment && window.confirm(memberT(document.documentElement.lang === "zh-Hant" ? "zh" : document.documentElement.lang, "members.updates.confirmDeleteComment"))) {
      const entry = state.updateLogs.find((item) => item.id === removeComment.getAttribute("data-update-id"));
