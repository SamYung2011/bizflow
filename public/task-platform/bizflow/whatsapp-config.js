...
function field({ e, label, value = "", field, type = "text", hint = "", disabled = false, min = null, max = null }) {
  return `<label class="wa-field"><span>${e(label)}</span><input type="${type}" value="${e(value)}" data-wa-setting="${e(field)}"${disabled ? " disabled" : ""}${min === null ? "" : ` min="${e(min)}"`}${max === null ? "" : ` max="${e(max)}"`}>${hint ? `<small>${e(hint)}</small>` : ""}</label>`;
}

function textarea({ e, label, value, field, rows = 8 }) {
  return `<label class="wa-field wa-field--textarea"><span>${e(label)}</span><textarea rows="${rows}" data-wa-setting="${e(field)}">${e(value || "")}</textarea></label>`;
}
...
function renderSettings(state, helpers, t) {
  ...
  const modePanel = section(e(t("aiMode")), `<div class="wa-choice-grid">${modes.map(([value, key]) => `<button type="button" class="wa-choice${s.claudeMode === value ? " is-active" : ""}" data-wa-mode="${value}" aria-pressed="${s.claudeMode === value}"><strong>${e(t(key))}</strong><small>${e(t("localChange"))}</small></button>`).join("")}</div>`);
  ...
  const runtimePanel = section(e(t("runtime")), `<div class="wa-form-grid">${field({ e, label: t("botName"), value: s.botName, field: "botName" })}${field({ e, label: t("botPhone"), value: s.botPhone, field: "botPhone" })}${field({ e, label: t("specialChat"), value: s.bossChatName, field: "bossChatName" })}${field({ e, label: t("replyDelay"), value: s.replyDelayBase, field: "replyDelayBase", type: "number", min: 0 })}${field({ e, label: t("cooldown"), value: s.cooldownMinutes, field: "cooldownMinutes", type: "number", min: 0 })}${field({ e, label: t("maxReplies"), value: s.maxRepliesPerMin, field: "maxRepliesPerMin", type: "number", min: 0 })}${field({ e, label: t("reportHour"), value: s.dailyReportHour, field: "dailyReportHour", type: "number", min: 0, max: 23 })}</div><div class="wa-button-row"><button type="button" class="wa-button" data-wa-save="runtime">${e(t("saveLocal"))}</button>${state.savedSection === "runtime" ? `<span class="wa-saved">${e(t("savedLocal"))}</span>` : ""}</div>`);
...
function renderMeta(state, helpers, t) {
  const { escapeHtml: e } = helpers;
  const s = state.settings;
  const connection = section(e(t("officialConnection")), `<div class="wa-form-grid">${field({ e, label: t("graphVersion"), value: s.metaGraphVersion, field: "metaGraphVersion", hint: t("graphHint") })}${field({ e, label: t("phoneNumberId"), value: s.metaPhoneNumberId, field: "metaPhoneNumberId", hint: t("phoneIdHint") })}${field({ e, label: t("wabaId"), value: s.metaWabaId, field: "metaWabaId", hint: t("wabaHint") })}${field({ e, label: t("webhookPassword"), value: WHATSAPP_MASK, field: "webhookPassword", disabled: true, hint: t("webhookHint") })}</div><div class="wa-button-row"><button type="button" class="wa-button" data-wa-save="meta">${e(t("saveLocal"))}</button>${state.savedSection === "meta" ? `<span class="wa-saved">${e(t("savedLocal"))}</span>` : ""}</div>`);
  const voice = section(e(t("tts")), `<label class="wa-toggle-row"><input type="checkbox" data-wa-setting="metaTtsEnabled"${s.metaTtsEnabled ? " checked" : ""}><span>${e(t("ttsEnabled"))}</span></label><div class="wa-form-grid">${field({ e, label: t("relayUrl"), value: s.metaTtsRelayUrl, field: "metaTtsRelayUrl" })}${field({ e, label: t("voiceId"), value: s.metaTtsVoiceId, field: "metaTtsVoiceId" })}${field({ e, label: t("languageBoost"), value: s.metaTtsLanguageBoost, field: "metaTtsLanguageBoost" })}</div>${textarea({ e, label: t("ttsPrompt"), value: s.metaTtsPrompt, field: "metaTtsPrompt", rows: 7 })}`);
...
function renderKnowledge(state, helpers, t) {
  ...
  return section(e(t("knowledgeTitle")), `<div class="wa-editor-meta"><span class="wa-dirty${dirty ? " is-dirty" : ""}" data-wa-knowledge-status>${e(t(dirty ? "unsaved" : "saved"))}</span><span data-wa-knowledge-count>${e(t("chars", { count: String(state.settings.knowledge || "").length }))}</span></div>${textarea({ e, label: t("knowledgeTitle"), value: state.settings.knowledge, field: "knowledge", rows: 24 })}<div class="wa-button-row"><button type="button" class="wa-button wa-button--secondary" data-wa-knowledge-cancel${dirty ? "" : " disabled"}>${e(t("cancelChanges"))}</button><button type="button" class="wa-button" data-wa-save="knowledge"${dirty ? "" : " disabled"}>${e(t("saveLocal"))}</button></div>`, "wa-panel--editor");
}

function renderChargerPrompt(state, helpers, t) {
  const { escapeHtml: e } = helpers;
  const valid = promptPlaceholdersValid(state.settings.locationHintPrompt);
  return section(e(t("chargerPromptTitle")), `${textarea({ e, label: t("chargerPromptTitle"), value: state.settings.chargersPrompt, field: "chargersPrompt", rows: 12 })}${textarea({ e, label: t("locationPromptTitle"), value: state.settings.locationHintPrompt, field: "locationHintPrompt", rows: 16 })}<p class="wa-validation${valid ? " is-valid" : " is-invalid"}" data-wa-prompt-validation>${e(t(valid ? "promptValid" : "placeholderMissing"))}</p><button type="button" class="wa-button" data-wa-save="prompts"${valid ? "" : " disabled"}>${e(t("saveLocal"))}</button>`, "wa-panel--editor");
}
...
function renderWhitelist(state, helpers, t) {
  ...
  return `<div class="wa-whitelist-grid">${whitelistKinds.map((kind) => {
    ...
    return section(e(t(labels[kind])), `<ul class="wa-whitelist-list">${rows.map((row) => `<li><label class="wa-toggle-row"><input type="checkbox" data-wa-whitelist-toggle="${e(row.id)}"${row.active ? " checked" : ""}><span>${e(t("active"))}</span></label><div><strong>${e(row.value || t("empty"))}</strong><small>${e(row.note || t("empty"))}</small></div><button type="button" class="wa-icon-button" data-wa-whitelist-remove="${e(row.id)}" title="${e(t("remove"))}">×</button></li>`).join("")}</ul><div class="wa-whitelist-add"><input value="${e(draft.value)}" data-wa-whitelist-draft="value" data-wa-whitelist-kind="${kind}" placeholder="${e(t("value"))}"><input value="${e(draft.note)}" data-wa-whitelist-draft="note" data-wa-whitelist-kind="${kind}" placeholder="${e(t("note"))}"><button type="button" class="wa-button" data-wa-whitelist-add="${kind}"${draft.value.trim() ? "" : " disabled"}>${e(t("add"))}</button></div>`);
  }).join("")}</div>`;
}
