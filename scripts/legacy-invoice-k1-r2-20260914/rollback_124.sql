-- Operator-only inverse of migration 124. It refuses post-merge edits instead of overwriting them.
...
DECLARE
  ...
  r1_backup_before jsonb; untouched_before jsonb;
...
BEGIN
  IF to_regclass('public.invoice_dedup_k1r2_20260914') IS NULL THEN
    RAISE EXCEPTION 'K1R2 rollback abort: backup table does not exist';
  END IF;
  ...
  SELECT jsonb_agg(to_jsonb(backup) ORDER BY old_id) INTO r1_backup_before FROM public.invoice_dedup_k1_20260914 backup;
  SELECT count(*),count(*) FILTER(WHERE rolled_back_at IS NULL) INTO backup_count,active_count
  FROM public.invoice_dedup_k1r2_20260914;
  IF backup_count=737 AND active_count=0 THEN
    RAISE NOTICE 'K1R2_ROLLBACK_ALREADY_DONE changed=0';
    RETURN;
  END IF;
  IF backup_count<>737 OR active_count<>737 THEN
    RAISE EXCEPTION 'K1R2 rollback abort: expected 737 active backup pairs, got % / %',backup_count,active_count;
  END IF;
  IF (SELECT count(*) FROM pg_constraint WHERE contype='f' AND confrelid='public.invoices'::regclass) <> 5 THEN
    RAISE EXCEPTION 'K1R2 rollback abort: inbound invoice FK schema changed';
  END IF;
  SELECT jsonb_agg(to_jsonb(i) ORDER BY id) INTO untouched_before FROM public.invoices i
  WHERE NOT EXISTS(SELECT 1 FROM public.invoice_dedup_k1r2_20260914 backup WHERE i.id IN(backup.old_id,backup.notion_id));
  ...
  FOR b IN SELECT * FROM public.invoice_dedup_k1r2_20260914 ORDER BY old_id LOOP
    ...
    IF current_row IS DISTINCT FROM b.old_row_after OR EXISTS(SELECT 1 FROM public.invoices WHERE id=b.notion_id) THEN
      RAISE EXCEPTION 'K1R2 rollback abort: old invoice drift or Notion id already exists (invoice %)',b.old_invoice_number;
    END IF;
    FOREACH t IN ARRAY fk_tables LOOP
      IF jsonb_array_length(b.fk_rows_before->t) <> (b.fk_repoints->>t)::int THEN
        RAISE EXCEPTION 'K1R2 rollback abort: corrupt FK backup in %',t;
      END IF;
      FOR r IN SELECT value FROM jsonb_array_elements(b.fk_rows_before->t) LOOP
        ...
        IF current_row IS DISTINCT FROM jsonb_set(r,'{invoice_id}',to_jsonb(b.old_id)) THEN
          RAISE EXCEPTION 'K1R2 rollback abort: repointed FK row drift in %',t;
        END IF;
      END LOOP;
    END LOOP;
  END LOOP;
  ...
  FOR b IN SELECT * FROM public.invoice_dedup_k1r2_20260914 ORDER BY old_id LOOP
    ...
    IF changed<>1 THEN RAISE EXCEPTION 'K1R2 rollback abort: Notion insert count=%',changed; END IF;
    SELECT to_jsonb(i) INTO current_row FROM public.invoices i WHERE id=b.notion_id;
    IF current_row IS DISTINCT FROM b.notion_row THEN RAISE EXCEPTION 'K1R2 rollback abort: Notion full row differs after restore'; END IF;
    ...
    FOREACH t IN ARRAY fk_tables LOOP
      FOR r IN SELECT value FROM jsonb_array_elements(b.fk_rows_before->t) LOOP
        ...
        GET DIAGNOSTICS changed=ROW_COUNT;
        IF changed<>1 THEN RAISE EXCEPTION 'K1R2 rollback abort: FK restore count in % = %',t,changed; END IF;
    ...
    IF changed<>1 OR current_row IS DISTINCT FROM b.old_row_before THEN
      RAISE EXCEPTION 'K1R2 rollback abort: retained old invoice restoration mismatch';
    END IF;
    restored := restored+1;
  END LOOP;
  ...
  FOREACH t IN ARRAY fk_tables LOOP
    ...
    LEFT JOIN (SELECT value AS r FROM public.invoice_dedup_k1r2_20260914 backup CROSS JOIN LATERAL jsonb_array_elements(backup.fk_rows_before->t)) saved
      ON saved.r->>'id'=e->>'id';
    IF (SELECT jsonb_agg(e ORDER BY e->>'id') FROM jsonb_array_elements(current_rows) e)
      ...
      RAISE EXCEPTION 'K1R2 rollback abort: child table restoration mismatch in %',t;
    END IF;
  END LOOP;
  SELECT count(*),COALESCE(sum(total) FILTER(WHERE status='Paid'),0) INTO count_after,amount_after FROM public.invoices;
  IF restored<>737 OR count_after<>count_before+737 OR amount_after<>amount_before+5278736.20 THEN
    RAISE EXCEPTION 'K1R2 rollback abort: totals mismatch restored=% before=% after=% paid_before=% paid_after=%',
      restored,count_before,count_after,amount_before,amount_after;
  END IF;
  IF (SELECT jsonb_agg(to_jsonb(i) ORDER BY id) FROM public.invoices i
      WHERE NOT EXISTS(SELECT 1 FROM public.invoice_dedup_k1r2_20260914 backup WHERE i.id IN(backup.old_id,backup.notion_id))) IS DISTINCT FROM untouched_before
     OR (SELECT jsonb_agg(to_jsonb(backup) ORDER BY old_id) FROM public.invoice_dedup_k1_20260914 backup) IS DISTINCT FROM r1_backup_before THEN
    RAISE EXCEPTION 'K1R2 rollback abort: unrelated invoices or R1 backup changed';
  END IF;
  UPDATE public.invoice_dedup_k1r2_20260914 SET rolled_back_at=now() WHERE rolled_back_at IS NULL;
  RAISE NOTICE 'K1R2_ROLLBACK_SUCCESS restored=% invoices=% paid=% fk_restored=% backup_retained=737',restored,count_after,amount_after,fk_restored;
END;
$rollback$;
COMMIT;
