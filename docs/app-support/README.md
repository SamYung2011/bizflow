...
1. 权限只放 `isBfAdmin`；处理中的默认含义为 open 且最后一条不是 user（与 waiting_staff 互补）。回复成功即认领，后回复的员工接手；失败或已保存消息的重试不改负责人。顶栏只显示负责人，不提供手动认领/释放；B6 只改 category。
...
support 专项 30/30：5 类会话、筛选/手机搜索、历史/增量消息、媒体描述、已读指针、失败重发幂等、回复认领与后回复接手、失败/旧消息重试不抢回负责人、分类更新只传 category、结束、合并去重、员工姓名、B1–B7 路径及请求体、非零业务码、上传分步与限制、mock 开关、权限拒绝、76 个界面文案键英法齐全、无中文 JSX 字面文案、新模块均少于 300 行。
