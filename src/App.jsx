    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "'DM Sans','Helvetica Neue',sans-serif", background: "#f7f8fc", color: "#1a1a2e" }}>
      ...
      <main style={{ flex: 1, overflow: "auto", padding: "32px 36px" }}>
        ...
        {tab === "employees" && (() => {
          ...
          const colBox = (title, color, count, tasksArr) => (
            <div style={{ background: "#fafbfc", border: "1px solid #eef0fa", borderRadius: 12, padding: 14, minHeight: 240, display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <span style={{ fontSize: 14, fontWeight: 700, color }}>{title}</span>
                <span style={{ fontSize: 12, color: "#aaa" }}>{count}</span>
              </div>
              <div style={{ flex: 1, overflowY: "auto" }}>{tasksArr.map((t, i) => renderTaskCard(t, i))}</div>
            </div>
          );
          return (
            <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 20, minHeight: "calc(100vh - 80px)" }}>
              {/* 左欄：員工列表 */}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          {editingEmpId === emp.id ? (
                            <>
                              <input value={emp.name || ""} onChange={e => setEmployees(prev => prev.map(x => x.id === emp.id ? { ...x, name: e.target.value } : x))} onBlur={async (e) => { const v = (e.target.value || "").trim() || null; const { error } = await supabase.from("employees").update({ name: v }).eq("id", emp.id); if (error) { alert(`${t("保存失敗")}：${error.message}`); return; } queryClient.setQueryData(["bf", "employees"], (old) => Array.isArray(old) ? old.map(x => x.id === emp.id ? { ...x, name: v } : x) : old); }} placeholder={t("姓名")} style={{ border: "1px solid #6382ff", outline: "none", padding: "6px 10px", borderRadius: 6, fontFamily: "inherit", fontSize: 18, fontWeight: 800, width: 200, marginBottom: 6, boxSizing: "border-box" }} />
                              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                                <input value={emp.role || ""} onChange={e => setEmployees(prev => prev.map(x => x.id === emp.id ? { ...x, role: e.target.value } : x))} onBlur={async (e) => { const v = (e.target.value || "").trim() || null; const { error } = await supabase.from("employees").update({ role: v }).eq("id", emp.id); if (error) { alert(`${t("保存失敗")}：${error.message}`); return; } queryClient.setQueryData(["bf", "employees"], (old) => Array.isArray(old) ? old.map(x => x.id === emp.id ? { ...x, role: v } : x) : old); }} placeholder={t("職位")} style={{ border: "1px solid #d0d0d0", outline: "none", padding: "5px 8px", borderRadius: 6, fontFamily: "inherit", fontSize: 12, width: 100 }} />
                                <input value={emp.phone || ""} onChange={e => setEmployees(prev => prev.map(x => x.id === emp.id ? { ...x, phone: e.target.value } : x))} onBlur={async (e) => { const v = (e.target.value || "").trim() || null; const { error } = await supabase.from("employees").update({ phone: v }).eq("id", emp.id); if (error) { alert(`${t("保存失敗")}：${error.message}`); return; } queryClient.setQueryData(["bf", "employees"], (old) => Array.isArray(old) ? old.map(x => x.id === emp.id ? { ...x, phone: v } : x) : old); }} placeholder={t("電話")} style={{ border: "1px solid #d0d0d0", outline: "none", padding: "5px 8px", borderRadius: 6, fontFamily: "inherit", fontSize: 12, width: 140 }} />
                                <input value={emp.email || ""} onChange={e => setEmployees(prev => prev.map(x => x.id === emp.id ? { ...x, email: e.target.value } : x))} onBlur={async (e) => { const v = (e.target.value || "").trim() || null; const { error } = await supabase.from("employees").update({ email: v }).eq("id", emp.id); if (error) { alert(`${t("保存失敗")}：${error.message}`); return; } queryClient.setQueryData(["bf", "employees"], (old) => Array.isArray(old) ? old.map(x => x.id === emp.id ? { ...x, email: v } : x) : old); }} placeholder="email" style={{ border: "1px solid #d0d0d0", outline: "none", padding: "5px 8px", borderRadius: 6, fontFamily: "inherit", fontSize: 12, width: 220 }} />
                              </div>
                            </>
                          ) : (
                            <>
                ...
                <div style={{ maxWidth: 720 }}>
                  <div style={{ background: "#fff", border: "1px solid #f0f0f0", borderRadius: 12, padding: 20, marginBottom: 16 }}>
                    ...
                    {(s.claude_mode === "api_cloud") && (() => {
                      const liveClients = waClients.filter(c => Date.now() - new Date(c.last_seen).getTime() < 10000);
                                ...
                                {liveClients.map(c => {
                                  ...
                                  return (
                                    <div key={c.client_id} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                      <span style={{ width: 6, height: 6, borderRadius: "50%", background: ageS < 5 ? "#22c55e" : "#f59e0b", display: "inline-block" }}></span>
