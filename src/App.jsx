                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          {editingEmpId === emp.id ? (
                            <>
                              <input value={emp.name || ""} onChange={e => setEmployees(prev => prev.map(x => x.id === emp.id ? { ...x, name: e.target.value } : x))} onBlur={async (e) => { const v = (e.target.value || "").trim() || null; const { error } = await supabase.from("employees").update({ name: v }).eq("id", emp.id); if (error) alert(`${t("保存失敗")}：${error.message}`); }} placeholder={t("姓名")} style={{ border: "1px solid #6382ff", outline: "none", padding: "6px 10px", borderRadius: 6, fontFamily: "inherit", fontSize: 18, fontWeight: 800, width: 200, marginBottom: 6, boxSizing: "border-box" }} />
                              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                                <input value={emp.role || ""} onChange={e => setEmployees(prev => prev.map(x => x.id === emp.id ? { ...x, role: e.target.value } : x))} onBlur={async (e) => { const v = (e.target.value || "").trim() || null; const { error } = await supabase.from("employees").update({ role: v }).eq("id", emp.id); if (error) alert(`${t("保存失敗")}：${error.message}`); }} placeholder={t("職位")} style={{ border: "1px solid #d0d0d0", outline: "none", padding: "5px 8px", borderRadius: 6, fontFamily: "inherit", fontSize: 12, width: 100 }} />
                                <input value={emp.phone || ""} onChange={e => setEmployees(prev => prev.map(x => x.id === emp.id ? { ...x, phone: e.target.value } : x))} onBlur={async (e) => { const v = (e.target.value || "").trim() || null; const { error } = await supabase.from("employees").update({ phone: v }).eq("id", emp.id); if (error) alert(`${t("保存失敗")}：${error.message}`); }} placeholder={t("電話")} style={{ border: "1px solid #d0d0d0", outline: "none", padding: "5px 8px", borderRadius: 6, fontFamily: "inherit", fontSize: 12, width: 140 }} />
                                <input value={emp.email || ""} onChange={e => setEmployees(prev => prev.map(x => x.id === emp.id ? { ...x, email: e.target.value } : x))} onBlur={async (e) => { const v = (e.target.value || "").trim() || null; const { error } = await supabase.from("employees").update({ email: v }).eq("id", emp.id); if (error) alert(`${t("保存失敗")}：${error.message}`); }} placeholder="email" style={{ border: "1px solid #d0d0d0", outline: "none", padding: "5px 8px", borderRadius: 6, fontFamily: "inherit", fontSize: 12, width: 220 }} />
                              </div>
                            </>
                          ) : (
                            <>
