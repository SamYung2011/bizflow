...
// 運營商流水 (OperatorMoneyLogs) — chargecms rc_customer_operator_money_log（list 唯讀，405 條）
// type 1=收入 / 2=提現（後端透傳 typeKey）；money 正負染色
...
export default function OperatorMoneyLogs({ session, isAdmin, active = true }) {
  const { t } = useT();
  ...
  const refresh = useCallback(async ({ force = false } = {}) => {
    ...
    try {
      ...
      const data = await callOcppAdmin(`/finance/operator-money-logs?${qs}`, { accessToken, force });
  ...
  return (
    <div>
      <FilterBar title={t("運營商流水")} onRefresh={refresh} loading={loading} count={rows.length}>
        ...
        <>
          <div style={TABLE_WRAP_STYLE}>
            <table style={TABLE_STYLE}>
              <TableHead columns={[t("類型"), t("運營商"), { label: t("金額"), align: "right" }, t("關聯訂單"), t("備註"), t("時間"), t("操作")]} />
              <tbody>
                {rows.map((r) => {
