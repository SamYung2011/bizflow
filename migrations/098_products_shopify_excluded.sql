-- 098: INV-bfonly-1 — subitems may remain in BizFlow while being absent from Shopify.
-- Deployment order: migration first, then shopify-catalog-write, then the front end.

ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS shopify_excluded boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.products.shopify_excluded IS
  'When true, this BizFlow subitem is deliberately absent from Shopify catalogue, links, and inventory sync.';
...
BEGIN
  ...
  INSERT INTO public.products (
    ...
    image_url, specs, product_type, collections, tags, parent_product_id,
    shopify_excluded, is_virtual
  ) VALUES (
    v_parent_id,
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    tags = EXCLUDED.tags,
    shopify_excluded = false;

  FOR v_variant IN
    SELECT *
    FROM jsonb_array_elements(
      COALESCE(v_product->'variants', '[]'::jsonb) ||
      COALESCE(v_product->'bizflowOnlyVariants', '[]'::jsonb)
    )
  LOOP
    ...
    INSERT INTO public.products (
      ...
      '{}'::text[],
      v_parent_id,
      COALESCE((v_variant->>'shopifyExcluded')::boolean, false),
      false
    )
    ON CONFLICT (id) DO UPDATE SET
      name = EXCLUDED.name,
      ...
      shopify_excluded = EXCLUDED.shopify_excluded;
  ...
  -- Remove the old links for this managed product before rebuilding the exact
  -- post-productSet variant map.
  ...
  -- Exclusion is stronger than the managed-product map: remove legacy direct
  -- columns and every M:N link so link-component cannot keep it on Shopify.
  UPDATE public.products
  SET shopify_variant_id = NULL,
      shopify_sku = NULL
  WHERE parent_product_id = v_parent_id
    AND shopify_excluded = true;

  DELETE FROM public.shopify_variant_links l
  WHERE l.bizflow_product_id IN (
    SELECT id
