-- 123: Merge only the 761 balanced K1 groups approved on 2026-09-14.
...
CREATE TABLE IF NOT EXISTS public.invoice_dedup_k1_20260914 (
  ...
  batch text NOT NULL DEFAULT 'k1-20260914' CHECK (batch = 'k1-20260914'),
...
ALTER TABLE public.invoice_dedup_k1_20260914 ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.invoice_dedup_k1_20260914 FROM PUBLIC, anon, authenticated, service_role;
...
BEGIN
  ...
  IF (SELECT count(*) FROM pg_constraint WHERE contype='f' AND confrelid='public.invoices'::regclass) <> 5
    ...
    RAISE EXCEPTION 'K1 abort: inbound invoice FK schema differs from the five reviewed tables';
  END IF;

  SELECT count(*) INTO backup_count FROM public.invoice_dedup_k1_20260914;
  IF backup_count > 0 THEN
    IF backup_count <> 989 OR EXISTS (
      SELECT 1 FROM public.invoice_dedup_k1_20260914 b
      ...
      RAISE EXCEPTION 'K1 abort: existing backup is partial, rolled back, or live rows have drifted (backup=%)', backup_count;
    END IF;
    RAISE NOTICE 'K1_ALREADY_APPLIED backup=989 changed=0';
    RETURN;
  END IF;

  SELECT jsonb_build_object('invoices',count(*),'unowned',count(*) FILTER (WHERE customer_id IS NULL),
    'notion',count(*) FILTER (WHERE notes LIKE '__NOTION_IMPORT__%' AND customer_id IS NOT NULL),
    'paid',COALESCE(sum(total) FILTER (WHERE status='Paid'),0))
  INTO before_stats FROM public.invoices;
  IF (before_stats->>'unowned')::int <> 3781 OR (before_stats->>'notion')::int <> 1843 THEN
    RAISE EXCEPTION 'K1 abort: cohort drift before mutation: %', before_stats;
  END IF;
  ...
  WITH normalized AS MATERIALIZED (
    ...
    SELECT k1,count(*) FILTER(WHERE side='L')::int AS group_size
    FROM keyed GROUP BY k1
    HAVING count(*) FILTER(WHERE side='L')>0
       AND count(*) FILTER(WHERE side='L')=count(*) FILTER(WHERE side='N')
  ), old_ranked AS (
    SELECT k.*,b.group_size,row_number() OVER(PARTITION BY k.k1 ORDER BY invoice_number ASC,id ASC)::int AS pair_rank
    FROM keyed k JOIN balanced b USING(k1) WHERE side='L'
  ), n_ranked AS (
    ...
    FROM keyed k JOIN balanced b USING(k1) WHERE side='N'
  )
  SELECT jsonb_agg(jsonb_build_object('old_id',o.id,'notion_id',n.id,'invoice_number',o.invoice_number,
    ...
    'group_size',o.group_size,'notion_idx',n.notion_idx) ORDER BY o.date,o.invoice_number,o.id),
    count(*),count(DISTINCT o.k1),sum(o.total)
  INTO pair_data,pair_count,group_count,amount_total
  FROM old_ranked o JOIN n_ranked n ON n.k1=o.k1 AND n.pair_rank=o.pair_rank;

  RAISE NOTICE 'K1_PREFLIGHT groups=% old=% notion=% amount=% before=%',
    group_count,pair_count,(SELECT count(DISTINCT e->>'notion_id') FROM jsonb_array_elements(pair_data) e),amount_total,before_stats;
  IF group_count IS DISTINCT FROM 761 OR pair_count IS DISTINCT FROM 989 OR amount_total IS DISTINCT FROM 6716667.00
     OR (SELECT count(DISTINCT e->>'notion_id') FROM jsonb_array_elements(pair_data) e) <> 989
     OR (SELECT count(DISTINCT e->>'old_id') FROM jsonb_array_elements(pair_data) e) <> 989
     OR EXISTS(SELECT 1 FROM jsonb_array_elements(pair_data) e WHERE e->>'invoice_number' IS NULL OR e->>'notion_idx' IS NULL) THEN
    RAISE EXCEPTION 'K1 abort: expected 761 groups / 989 old / 989 Notion / 6716667.00 and valid sort keys';
  END IF;
  ...
  FOR p IN SELECT * FROM jsonb_to_recordset(pair_data) AS x(old_id text,notion_id text,invoice_number int,
      date date,total numeric,fingerprint text,pair_rank int,group_size int,notion_idx bigint)
  LOOP
    ...
    IF marker IS NULL OR marker !~ '^__NOTION_IMPORT__ batch=\S+ idx=[0-9]+ raw_status=\S+$'
      ...
      RAISE EXCEPTION 'K1 abort: pair fields/marker drift at old invoice %',p.invoice_number;
    END IF;
    ...
    INSERT INTO public.invoice_dedup_k1_20260914(old_id,old_invoice_number,notion_id,group_date,group_total,
      group_fingerprint,pair_rank,group_size,old_customer_id_before,notion_row,fk_repoints,
      fk_rows_before,old_row_before,old_row_after,marker_line,run_stats)
    VALUES(p.old_id,p.invoice_number,p.notion_id,p.date,p.total,p.fingerprint,p.pair_rank,p.group_size,
    ...
    FOREACH t IN ARRAY fk_tables LOOP
      ...
      IF changed <> (fk_counts->>t)::int THEN RAISE EXCEPTION 'K1 abort: FK repoint mismatch in %',t; END IF;
    END LOOP;
    UPDATE public.invoices SET customer_id=(n_row->>'customer_id')::uuid WHERE id=p.old_id AND customer_id IS NULL;
    GET DIAGNOSTICS changed=ROW_COUNT;
    IF changed <> 1 THEN RAISE EXCEPTION 'K1 abort: customer update count=%',changed; END IF;
    ...
    IF changed <> 1 OR (SELECT to_jsonb(i) FROM public.invoices i WHERE id=p.old_id) IS DISTINCT FROM expected_old THEN
      RAISE EXCEPTION 'K1 abort: old invoice changed outside customer_id/notes';
    END IF;
    DELETE FROM public.invoices WHERE id=p.notion_id;
    GET DIAGNOSTICS changed=ROW_COUNT;
    IF changed <> 1 THEN RAISE EXCEPTION 'K1 abort: delete count=%',changed; END IF;
  END LOOP;

  SELECT count(*) INTO original_n_count FROM public.invoices i
  WHERE notes LIKE '__NOTION_IMPORT__%' AND customer_id IS NOT NULL
    AND NOT EXISTS(SELECT 1 FROM public.invoice_dedup_k1_20260914 b WHERE b.old_id=i.id);
  SELECT count(*) INTO inherited_count FROM public.invoices i JOIN public.invoice_dedup_k1_20260914 b ON b.old_id=i.id
  ...
  RAISE NOTICE 'K1_POSTCHECK %',after_stats;
  IF (after_stats->>'invoices')::int <> (before_stats->>'invoices')::int-989
     OR (after_stats->>'unowned')::int <> 2792 OR original_n_count <> 854 OR inherited_count <> 989
     OR (after_stats->>'paid')::numeric <> (before_stats->>'paid')::numeric-6716667.00
     OR new_backup <> 989 OR (SELECT count(*) FROM public.invoice_dedup_k1_20260914) <> 989 THEN
    RAISE EXCEPTION 'K1 abort: postcheck failed: %',after_stats;
  END IF;
  IF (SELECT jsonb_agg(to_jsonb(i) ORDER BY i.id) FROM public.invoices i
      WHERE NOT EXISTS(SELECT 1 FROM public.invoice_dedup_k1_20260914 b WHERE b.old_id=i.id)) IS DISTINCT FROM untouched_before THEN
    RAISE EXCEPTION 'K1 abort: unrelated invoice rows changed';
  END IF;
  FOREACH t IN ARRAY fk_tables LOOP
    EXECUTE format('SELECT count(*) FROM public.%I x JOIN public.invoice_dedup_k1_20260914 b ON x.invoice_id=b.notion_id',t) INTO actual_count;
    IF actual_count <> 0 THEN RAISE EXCEPTION 'K1 abort: dangling Notion FK in %',t; END IF;
    EXECUTE format('SELECT COALESCE(jsonb_agg(to_jsonb(x) ORDER BY id),''[]''::jsonb) FROM public.%I x',t) INTO current_rows;
    ...
    FROM jsonb_array_elements(fk_before->t) e LEFT JOIN public.invoice_dedup_k1_20260914 b ON b.notion_id=e->>'invoice_id';
    ...
    IF (SELECT jsonb_agg(e ORDER BY e->>'id') FROM jsonb_array_elements(current_rows) e)
      ...
      RAISE EXCEPTION 'K1 abort: FK rows changed beyond invoice_id in %',t;
    END IF;
    SELECT COALESCE(sum((fk_repoints->>t)::int),0) INTO changed FROM public.invoice_dedup_k1_20260914;
    RAISE NOTICE 'K1_FK table=% repointed=% remaining_notion_refs=%',t,changed,actual_count;
  END LOOP;
  RAISE NOTICE 'K1_SUCCESS groups=761 old=989 notion=989 amount=6716667.00 backup=989 unique_old=989';
END;
$merge$;
COMMIT;
